...
pdf(paste(func,'.pdf',sep=''), width = 5, height = 5)
all_errs <- c(do.call(rbind, res))
plot(NA,NA,xlim=range(Ns), ylim = range(log10(all_errs)), xlab = 'Sample Size', ylab = 'Log Cosine', main = pn_func[[func]])
for (covtype in covtypes) {
    med <- apply(log10(res[[covtype]]), 2, median)
